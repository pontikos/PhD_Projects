...
make.prior <- function(fcs.data, K=4, level=.5, B=500, channels=NULL, plot.file=NULL, nu=4, pool.file=NULL) {
    num.clusters <- 0
    d <- fcs.data
    ...
    #}
    #prior <- list(prior=flowClust2Prior(res,kappa=1),res=res,data=d)
    prior <- flowClust2Prior(res,kappa=1)
    #save(prior, file=out.file, compress='xz', compression_level=9)
...
### 
### 
gate <- function(d, fcs.name=NULL, level=.9, u.cutoff=.5, B=500, post.threshold=.99, prior=NULL, plot.file=NULL, out.file=NULL, kappa=1) {
    prior$prior <- flowClust2Prior(prior$res,kappa=kappa)
    cat('Prior\n')
    print(prior$prior)
    print(prior$res)
    varNames <- prior$res@varNames
    if (file.exists(out.file)) {
        cat(out.file, 'exists not running again!\n')
        print(load(out.file))
        print(fcs.name)
    } else {
        res <- flowClust::flowClust(d,varNames=varNames,K=prior$res@K,B=B,usePrior='yes',prior=prior$prior,level=level,u.cutoff=u.cutoff,trans=0,lambda=1,control=list(B.lambda=0))
        result <- list(res=res, prior=prior, fcs.data=d, fcs.name=fcs.name)
        save(result, file=out.file, compress='xz', compression_level=9)
        print(fcs.name)
    }
    print(head(d <- d[,varNames]))
    if (!is.null(plot.file)) {
        cat('Plot\n')
        print(prior.plot.file <- gsub('.png','-prior.png',plot.file))
        print(posterior.plot.file <- gsub('.png','-posterior.png',plot.file))
        plotClustRes(d, prior$res, outliers=TRUE, plot.file=prior.plot.file)
        plotClustRes(d, res, plot.file=posterior.plot.file)
    }
    # assignment to whichever clusters has the highest posterior probability
    clusters <- MAP(d,res)
    ...
    print(num.clusters <- length(table(clusters)))
    cat('>>weights', res@w, '\n' )
    cat('>>weights', table(clusters)/length(clusters), '\n')
    return(result)
}
...
option_list <- list( 
    ...
    make_option(c("-f","--in.file"), default=NULL, help = ''),
    ...
    make_option(c('--channels'), default='CD25,CD45RA,CD4,FOXP3', help='Channels on which to do the clustering'),
    make_option(c('-K', '--clusters'), default=6, help='Number of clusters for prior.  The default is %default.'),
...
if (is.null(opt$prior)) {
    ...
    prior <- make.prior(fcs.data, channels=channels, K=opt$clusters, plot.file=prior.plot.file, B=opt$em.iterations, level=opt$level, nu=4)
    print( prior$prior )
