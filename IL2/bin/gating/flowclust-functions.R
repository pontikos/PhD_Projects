...
make.prior <- function(d, K, kappa=100, Nt=NULL) {
    print(sort(unique(K)))
    p <- ncol(d)
...
#
flowClustering <- function(d, prior=prior, level=.95, u.cutoff=.1, B=1000) {
    print(res <- flowClust::flowClust(d,varNames=colnames(d),K=prior$K,B=B,level=level,u.cutoff=u.cutoff,trans=0,lambda=1,control=list(B.lambda=0),usePrior='yes',prior=prior))
    return(flowClust::Map(res))
}
