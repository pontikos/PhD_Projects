...
### gating plot
print(file.path('~nikolas/dunwich/Projects/IL2/PSTAT5-CD25-CD45RA-CD4-FOXP3','flowclust-manual-gates','Plots',sprintf('%s.pdf',gsub('.RData','',basename(file.name)))))
pdf(file.path('~nikolas/dunwich/Projects/IL2/PSTAT5-CD25-CD45RA-CD4-FOXP3','flowclust-manual-gates','Plots',sprintf('%s.pdf',gsub('.RData','',basename(file.name)))))
...
# CD4+ lymphocytes
#
res <- flowClust::flowClust(fcs.data,varNames=c('CD4','SSCA'),K=2,B=B,level=level,u.cutoff=u.cutoff,trans=0,lambda=1,control=list(B.lambda=0))
classification <- MAP(fcs.data, res)
cat('CD4\n')
cd4 <- as.numeric(classification==which.max(tapply(fcs.data[,'CD4'], classification, mean)))
classification <- MAP(fcs.data, res)
...
# memory / naive
res <- flowClust::flowClust(fcs.data,varNames=c('CD45RA','SSCA'),K=2,B=B,level=level,u.cutoff=u.cutoff,trans=0,lambda=1,control=list(B.lambda=0))
classification <- MAP(fcs.data, res)
cat('CD45RA\n')
...
#load prior
print(load('~/dunwich/Projects/IL2/PSTAT5-CD25-CD45RA-CD4-FOXP3/flowclust-eff-treg-prior.RData'))
# naive effector / treg
cat('Naive\n')
res <- flowClust::flowClust(naive,varNames=c('CD25','FOXP3'),K=2,B=B,level=level,u.cutoff=u.cutoff,trans=0,lambda=1,control=list(B.lambda=0),usePrior='yes',prior=prior)
...
cat('Memory\n')
res <- flowClust::flowClust(memory,varNames=c('CD25','FOXP3'),K=2,B=B,level=level,u.cutoff=u.cutoff,trans=0,lambda=1,control=list(B.lambda=0),usePrior='yes',prior=prior)
...
save( CLR, file=file.path('~nikolas/dunwich/Projects/IL2/PSTAT5-CD25-CD45RA-CD4-FOXP3','flowclust-manual-gates','CLR',basename(file.name)) )
